...
void arrange_points() {
    ...
    for (int i = 1; i < (int) points.size(); i++) {
        while (i < (int) points.size() - 1 && orientation(points[0], points[i], points[i + 1]) == 0) {
            i++;
        }
        points[viable_points++] = points[i];
    }
    
    while ((int) points.size() > viable_points) {
        points.pop_back();
    }
}
...
void graham_scan() {
    ...
    pair<int, int> last = convex_hull.top();
    pair<int, int> first = last;
    convex_hull.pop();
    
    double ans = 0.0;
    while (convex_hull.empty() == false) {
        ans += sqrt(dist(last, convex_hull.top()));
        last = convex_hull.top();
        convex_hull.pop();
    }
    ans += sqrt(dist(last, first));
    cout << ans << "\n";
}
